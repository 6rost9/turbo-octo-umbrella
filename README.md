...
Прототип используется только для чтения свойств, операции записи или удлаения свойств работают с объектом напряимую. Задав метод для объекта, его наличие в прототипе не будет проверяться.
...
Почти все остальные методы, получающие ключи/значения, такие как `Object.keys`, `Object.values` и другие – игнорируют унаследованные свойства. Для проверки, что свойство является собственным можно использовать метод `O.hasOwnProperty()`;
